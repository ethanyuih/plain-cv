      ...
      titles: [
        {
          ...
          taskList: [
            ...
            // "Developed new Multi-Tenant RESTful APIs using .NET Core + EF Core, successfully migrating from the legacy system to PostgreSQL database, achieving a 500% improvement in average response time.",
            "Built RESTful API endpoints using .NET 8.0 with C#, achieving a 500% improvement in API response time. (.NET 8.0, C#)",
            "Designed and implemented multi-tenant database architecture using PostgreSQL and Entity Framework Core. (PostgreSQL, EFCore)",
            "Utilized Duende to integrate OAuth for secure authentication and JWT for stateless authorization, implementing role-based access control (RBAC) with middleware for request authorization, improving application security by 35%. (JWT, OAuth, Duende)",
            "Architected and led the decomposition of monolithic application into 8+ microservices on Kubernetes, reducing deployment time by 70%. (Kubernetes)",
            "Designed and built a central API Gateway with Go, leveraging ETCD's watch mechanism for service discovery, eliminating polling overhead and reducing service lookup latency by 55%. (Go, ETCD)",
            "Implemented gRPC with Go for inter-service communication across 10+ service endpoints, resulting in 40% reduced service latency and 30% decreased network bandwidth usage. (gRPC, Go)",
            "Enhanced database performance through query optimization by implementing proper indexing, query rewriting, and PostgreSQL VACUUM scheduling, resulting in a 40% improvement. (PostgreSQL, VACUUM)",
            ...
            "Established automated CI/CD workflow with Jenkins and Docker, enabling seamless code integration and deployment while reducing deployment time by 80%. (CI/CD, Jenkins, Docker)",
            ...
            "Architected robust cloud infrastructure using AWS S3 for media storage and AWS CloudWatch for logging. (AWS S3, CloudWatch)",
        ...
        },
        {
          ...
          taskList: [
            ...
            "Engineered real-time trade monitoring system using .NET 8.0 with C#, leveraging AWS SQS and Kafka for distributed message processing, achieving 100K messages/second throughput and sub-50ms trading feedback latency. (C#, .NET 8.0, AWS SQS, Kafka)",
            "Built and maintained third party payment integrations, preventing duplicate transactions through Redis-based distributed locking, achieving 99.99% processing accuracy. (Redis, Distributed Lock)",
            // "Spearheaded ChatGPT API implementation across core business applications, driving 20% increase in user satisfaction through enhanced automation.",
            "Utilized Hangfire for background jobs (reports, emails), achieving 60% reduction in server load. (Hangfire)",
      ...
      titles: [
        {
          ...
          taskList: [
            "Optimized x265 image encoding performance using C++ and SIMD instructions (SSE/AVX, NEON), implementing JavaScript APIs for data packing, resulting in 60% faster encoding speeds. (C++, SIMD)",
            "Designed and implemented  image conversion pipeline using x265 and libheif, reducing encoding time by 36% and achieving 22% better compression compared to WebP format. (libheif)",
            "Applied optimized image format into Meituan App using React Native. (React Native)",
          ],
        },
      ],
    },
