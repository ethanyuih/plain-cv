      ...
      titles: [
        {
          ...
          taskList: [
            ...
            'Built and deployed high-performance RESTful API endpoints for our CRM on AWS EC2 using Spring Boot 3.0 with Java 17, implementing Nginx for load balancing and reverse proxy with HTTP/2 support, while leveraging Spring Cache and Redis for data caching, achieving a 500% improvement in API response time and handling 10K concurrent requests.',
            'Architected a multi-tenant database architecture using PostgreSQL and Spring Data JPA with Hibernate as ORM provider.',
            "Utilized Spring Security with OAuth2 for secure authentication and JWT for stateless authorization, implementing role-based access control (RBAC) with middleware for request authorization, while determining user's tenant affiliation by extracting tenant identifiers from JWT claims in the multi-tenant middleware, eliminating the need for URL-based tenant identification.",
            'Designed and developed a unified wallet management system using Finite State Machines (FSM) with explicit Matter-State-Action architecture to efficiently handle transactions, including withdrawals, deposits, credit adjust and rebate commissions.',
        ...
        },
        {
          ...
          taskList: [
            ...
            'Integrated 17 third-party payment gateways and corresponding webhook APIs by Spring Boot, enabling seamless financial transactions.',
            ...
            'Implemented distributed job scheduling system using Spring Batch and Quartz, featuring dynamic cron expressions, job chaining and custom retry policies with exponential backoff, reducing peak server load by 60% through intelligent task distribution.',
